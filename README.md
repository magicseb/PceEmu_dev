...
- Sauvegarde d'état : l'empreinte de la ROM est stockée dans le fichier, ce qui interdit de charger l'état d'un autre jeu ; le verrou d'écriture de la BRAM ($1803/$1807) est émulé depuis la 1.40
...
- **1.40** — **audit sémantique complet** du CPU (contre le source MAME `h6280.cpp`, lu ligne à ligne) et de la partie ADPCM/CD-ROM (contre Mednafen `pcecd.cpp`), à la demande de l'utilisateur, sans symptôme préalable. **CPU** : **(1)** refonte du **flag T** au modèle matériel — T est le bit `$20` de P, posé par `SET`, effacé par toute instruction sauf `SET`/`CSL`/`CSH` (qui le préservent) et `PLP`/`RTI` (dont le P dépilé fait foi) ; une interruption pousse P **avec T** (B=0) et le préserve, si bien que **le mode T survit à une IRQ tombée entre `SET` et l'instruction ALU** — l'ancien modèle « ombre » le perdait, ce qui aurait corrompu A au retour d'interruption ; **(2)** **`SBC` a désormais sa variante en mode T** (opère sur `($2000+X)`, A intact — elle manquait entièrement) ; **(3)** cycles du mode T : **+3** (+1 en décimal, comme l'ADC/SBC décimal normal) ; **(4)** **`TMA #masque`** : le **dernier** bit du masque gagne (pas le premier) et TMA **ne modifie aucun drapeau** ; **(5)** **`PHP`** pousse P **sans** T (effacé avant le push) ; **(6)** **`PLP`** force B à 1 et arme le **délai d'un cran** de reconnaissance d'IRQ quand il démasque I (comme `CLI`, 1.34) ; `RTI` force B sans délai ; **(7)** `BRK` efface T avant le push. **CD-ROM/ADPCM** : **(8)** **verrou d'écriture de la BRAM** (le « reste à faire » historique) : lire `$1803` **verrouille** la BRAM (lectures `$FF`, écritures ignorées — jeux CD seulement, les HuCard gardent l'accès libre, comme beetle-pce), écrire `$1807` bit 7 la déverrouille (bit 7 à 0 : sans effet) ; **(9)** **fader `$180F`** : fondu de sortie CD-DA ou ADPCM (bit 3 démarrer, bit 2 rapide 2,5 s sinon 6 s, bit 1 cible ADPCM sinon CD-DA), volume 65536→0 ; **(10)** le bit `$02` de `$1803` (sélecteur gauche/droite) **alterne à chaque lecture**, et les ports **`$1805`/`$1806`** rendent l'échantillon CD-DA courant (valeur absolue, fondu appliqué) ; `$1804` se relit (dernière valeur écrite) ; la ligne d'IRQ est masquée par `$7C` ; **(11)** blindage : une commande READ visant un LBA hors du disque rend des zéros au lieu de planter l'émulateur. Sauvegardes d'état : **format 5** (fader, canal L/R, `$1804` ; formats 1-4 lisibles ; le format CPU est inchangé, compatible dans les deux sens). Non-régression : suite **177 tests, 0 échec** dont deux bancs enrichis — nouveau `Tests/CpuFlags` (7 cas : chaîne mode T complète ADC→SBC→ORA→AND→EOR, survie de T via RTI, TMA, PHP — 4 mutations ciblées échouent chacune sur son cas) et `Tests/CdRom` porté à 41 cas (verrou BRAM, bit L/R, fader — 2 mutations détectées) ; les quatre jeux de référence sont **au pixel près** identiques et les scénarios de reset/jingle de Down Load 2 revérifiés.
- **1.39** — correctif du **jingle ADPCM coupé pendant le logo NEC Avenue de Down Load 2** (comparaison vidéo RetroArch fournie : le son doit accompagner toute l'animation du logo ; chez nous il se coupait au tiers). Le jeu **streame** le jingle en ping-pong : la lecture ADPCM décompte la longueur pendant que des DMA CD rechargent la RAM, cadencés par l'IRQ « **moitié** » (`$04` de `$1803`, enables `$0C` actifs). Trois comportements manquaient, portés depuis le source Mednafen (`pcecd.cpp`, vérifié ligne à ligne) : **(1)** écrire un octet en RAM ADPCM (port `$180A` ou DMA CD) **ré-incrémente la longueur restante** (plafond `$FFFF`, si bit 4 non tenu) — c'est le mécanisme qui entretient le flux ; sans lui, la longueur s'épuisait et notre IRQ de fin (1.36) déclenchait le ménage du BIOS en plein jingle ; **(2)** le bit « moitié » est désormais émis : `longueur < $8000`, recalculé à chaque fetch de lecture, écriture RAM et lecture CPU, forcé à faux quand la fin est atteinte, au reset et au démarrage d'une lecture ; **(3)** la relecture CPU par `$180A` **décrémente aussi la longueur** (et pose la fin à zéro). Les bits `$04/$08` de `$1803` sont maintenant **composés en direct** depuis les drapeaux (niveaux, comme le matériel) au lieu d'un bit collant. Sauvegardes d'état : **format 4** (drapeau moitié ; formats 1-3 lisibles). Vérifié : le jingle tient ~12 s en continu à travers tout le logo puis laisse place à la musique du titre. Non-régression : suite **164 tests, 0 échec** (+6 cas streaming dans `Tests/CdRom`, mutations ciblées : sans l'incrément d'écriture ou sans le calcul de moitié, les cas dédiés échouent) ; Down Load 2 (boot complet), Forgotten Worlds (gros consommateur de relecture ADPCM), Turrican, D&D et les scénarios de reset (1.38) revérifiés.
- **1.38** — correctif de la **piste CD-DA qui survivait au reset** (signalé sur Down Load 2 : après une réinitialisation de la console, la musique du titre continuait de jouer par-dessus l'écran de boot de la System Card, et même après relancement du jeu). Deux comportements du lecteur manquaient, tous deux conformes au matériel (et à Mednafen) : le **RST SCSI** (`$1804` bit 1, que la System Card assert en redémarrant) fait tout avorter côté lecteur, **lecture audio comprise** ; et une commande **READ(6)** interrompt la lecture CD-DA en cours (le lecteur ne peut pas lire des secteurs et jouer de l'audio en même temps). Correctif (`Core/CdRom.vb`) : `ResetBus` et le handler READ arrêtent désormais la lecture CD-DA. Vérifié sur les deux sauvegardes d'état fournies : reset → la musique se coupe dès le RST du boot ; état « boot avec musique fantôme » → un appui sur RUN coupe la musique au premier READ et le jeu redémarre proprement (intro complète). Note : charger un état **sauvé pendant que la musique jouait** restaure la musique — c'est le comportement attendu d'une sauvegarde d'état. Non-régression : suite complète **158 tests, 0 échec**, dont 4 nouveaux cas dans `Tests/CdRom` (démarrage CD-DA vérifié par READ SUB-Q, interruption par RST, interruption par READ — prouvés sensibles par mutation) ; Down Load 2, Forgotten Worlds, Turrican et D&D revérifiés au pixel près.
- **1.37** — **audit CPU** (session d'audit sans symptôme préalable), trois correctifs `Core/Cpu6280.vb` / `Core/MemoryMap.vb` : **(1)** ordre de priorité des interruptions remis dans le sens du matériel — **TIMER > IRQ1 > IRQ2** (MAME h6280) ; le code testait IRQ2 en premier, ce qui ne change l'arbitrage que lorsque plusieurs IRQ coïncident sur la même instruction ; **(2)** la lecture du registre de statut d'IRQ `$1403` expose désormais aussi la ligne **IRQ2** (bit 0) — la prise d'interruption réelle n'était pas affectée, mais un programme qui interroge `$1403` directement ratait le CD ; **(3)** l'**ADC en mode décimal** effaçait à tort le drapeau **V**, qui doit rester intact (comme pour le SBC décimal, conformément au matériel). Ces trois correctifs ont été fusionnés dans la lignée principale avec la 1.38 ; la suite complète et les quatre jeux de référence ont été revérifiés sur l'arbre fusionné.
...
**Version** : 1.40 (août 2026) — audit sémantique CPU (MAME h6280 : flag T complet) et CD (Mednafen : verrou BRAM, fader $180F)  
